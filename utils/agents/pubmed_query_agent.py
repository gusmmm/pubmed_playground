...
The agent guides users through creating PubMed-compatible queries by:
- Converting natural language questions to search syntax
- Applying appropriate clinical filters
- Recommending filters based on the query context
- Removing redundant terms from the query

This agent is intended to be used before the PubMed Research Agent to
ensure optimal search results.
"""
...
# Configure logging
logger = logging.getLogger("pubmed.agents.query")


class PubMedQueryAgent(BaseAgent):
    """
    Agent that helps users construct optimized PubMed search queries.
    ...
    def run(self, query: Optional[str] = None) -> str:
        ...
        Args:
            query: Optional initial query. If None, will prompt the user.
        ...
        try:
            ...
            # Get natural language query from user or use provided one
            if query:
                natural_query = query
                self.console.print(f"\n[bold cyan]Using provided query:[/bold cyan] {natural_query}")
            else:
...
def run_pubmed_query_agent() -> str:
    ...
    try:
        ...
        agent = PubMedQueryAgent(console=console)
        return agent.run()
